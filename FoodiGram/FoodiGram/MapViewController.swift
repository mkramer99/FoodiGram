...
// take postArray and send to segue for collectionview
// fetch results of all posts
// display ones that match the strings in the array
...
class MapViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        view.bringSubviewToFront(listViewButton)
    ...
    func postAnnotations() {
        ...
        coffeeRequest.region = self.mapView.region
        bakeryRequest.region = self.mapView.region
        ...
        restaurantSearch.start { response , error in
            ...
            for item in response.mapItems {
                ...
                // show annotations
                self.postArray.append(annotation.title!)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        // send post array to display visile mapview location posts
        if segue.destination is MapViewPostsViewController {
            let san = segue.destination as? MapViewPostsViewController
            san?.postArray = postArray
        }
        if segue.destination is UserPostsViewController {
            let san = segue.destination as? UserPostsViewController
            san?.postArray = postArray
            let sac = segue.destination as? UserPostsViewController
            sac?.whichLoadImages = "list"
            let button = segue.destination as? UserPostsViewController
            button?.deletePost = false
